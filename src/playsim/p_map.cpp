...
int AimAssistTrace(AActor *t1, DAngle angle, double distance, DAngle pitch, int flags, double sz,
                   FLineTraceData *outdata)
{
	FTraceResults trace;
	...
	bool ret =
		Trace(startpos, t1->Level->PointInSector(startpos), {pc * angle.Cos(), pc * angle.Sin(), -pitch.Sin()},
	          distance, MF_AIMASSISTTARGET, ML_BLOCKEVERYTHING, t1, trace, TRACE_NoSky, CheckAimAssistTrace, nullptr);
